        ...
        <div class="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="stokopname_id" id="stokopname_id" class="form-select w-full" required>
                    ...
                    @foreach($stokopname as $item)
                        <option value="{{ $item->id }}" {{ $item->id == $barangkeluar->stokopname_id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="text" id="harga_satuan_display" class="form-input w-full"
                       value="{{ number_format($barangkeluar->harga_satuan, 0, ',', '.') }}" required>
                <input type="hidden" name="harga_satuan" id="harga_satuan" value="{{ $barangkeluar->harga_satuan }}">
            </div>
            ...
            <div>
                ...
                <input type="number" name="jumlah" id="jumlah" class="form-input w-full"
                       value="{{ $barangkeluar->jumlah }}" min="1" required>
            </div>
            ...
            <div>
                ...
                <input type="text" id="total_display" class="form-input w-full bg-gray-100"
                       value="{{ number_format($barangkeluar->total, 0, ',', '.') }}" readonly>
                <input type="hidden" name="total" id="total" value="{{ $barangkeluar->total }}">
            </div>
        </div>
