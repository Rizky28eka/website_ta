...
<div class="card">
    <div class="card-header flex justify-between items-center">
        ...
        <a href="{{ route('barangkeluar.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
            + Tambah Data
        </a>
    </div>
    ...
    <div class="p-6 overflow-x-auto">
        <table class="min-w-full border border-gray-200 divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($barangKeluar as $index => $bm)
                    <tr>
                        ...
                        <td class="px-4 py-2 text-sm">
                            <a href="{{ route('barangkeluar.edit', $bm->id) }}" class="px-3 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600">
                                Edit
                            </a>
                            <form action="{{ route('barangkeluar.destroy', $bm->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin hapus data ini?')">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-3 text-center text-gray-500">Tidak ada data Barang Keluar.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
